...
public class Canvas extends JPanel  {
    ...
    private class MouseEventHandler implements MouseListener {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            // checks if the pressed button is circle rectangle.
            ...
            else if (currentShape.equals("rotatedRect")){
                // we get the x coordinate
               pointX = e.getX();
               ...
               // get the last rectangle object from the list and rotate it.
               Shape myRect = circlesAndRectangles.get( circlesAndRectangles.size()-1);
               AffineTransform at = AffineTransform.getRotateInstance(Math.PI / 4, pointX, pointY);
               Shape rotatedRect = at.createTransformedShape(myRect);

                // removes the last Rectangle object from the array list we had for the shapes (circles and rectangle)
                // so we can add the rotated version
               circlesAndRectangles.remove(circlesAndRectangles.size()-1);

                // adds the new rotated Rectangle object to the array list we had for the shapes (circles and rectangle)
               circlesAndRectangles.add(rotatedRect);
